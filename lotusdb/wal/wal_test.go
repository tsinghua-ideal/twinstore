...
func TestWAL_Write(t *testing.T) {
	...
	// write 1
	pos1, err := wal.Write([]byte("hello1"), nil)
	assert.Nil(t, err)
	assert.NotNil(t, pos1)
	pos2, err := wal.Write([]byte("hello2"), nil)
	assert.Nil(t, err)
	assert.NotNil(t, pos2)
	pos3, err := wal.Write([]byte("hello3"), nil)
	assert.Nil(t, err)
	assert.NotNil(t, pos3)

	val, _, err := wal.Read(pos1)
	assert.Nil(t, err)
	assert.Equal(t, "hello1", string(val))
	val, _, err = wal.Read(pos2)
	assert.Nil(t, err)
	assert.Equal(t, "hello2", string(val))
	val, _, err = wal.Read(pos3)
	assert.Nil(t, err)
...
func TestWAL_OpenNewActiveSegment(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		pos, err := wal.Write([]byte(val), nil)
		assert.Nil(t, err)
		assert.NotNil(t, pos)
	}
}
...
func TestWAL_Reader(t *testing.T) {
	...
	validate := func(walInner *WAL, size int) {
		...
		for {
			chunk, _, position, err := reader.Next()
			if err != nil {
				if err == io.EOF {
					break
				}
...
func testWriteAllIterate(t *testing.T, wal *WAL, size, valueSize int) {
	for i := 0; i < size; i++ {
		...
		wal.PendingWrites([]byte(val), nil)
	}
...
func TestWAL_RenameFileExt(t *testing.T) {
	...
	for i := 0; i < 20000; i++ {
		_, err = wal2.Write([]byte(strings.Repeat("W", 512)), nil)
		assert.Nil(t, err)
	}
}
