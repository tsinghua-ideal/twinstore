...
func Test_bptreeIterator(t *testing.T) {
	...
	bt, err := openBTreeIndex(options)
	require.NoError(t, err)
	m := map[string]*dwal.ChunkPosition{
	...
	m2 := map[string]*dwal.ChunkPosition{
	...
	keyPositions = append(keyPositions, &KeyPosition{
		key:       []byte("key 0"),
		partition: 0,
		position:  &dwal.ChunkPosition{SegmentId: 0, BlockNumber: 0, ChunkOffset: 0, ChunkSize: 0},
	}, &KeyPosition{
		key:       []byte("key 1"),
		partition: 0,
		position:  &dwal.ChunkPosition{SegmentId: 1, BlockNumber: 1, ChunkOffset: 1, ChunkSize: 1},
	}, &KeyPosition{
		key:       []byte("key 2"),
		partition: 0,
		position:  &dwal.ChunkPosition{SegmentId: 2, BlockNumber: 2, ChunkOffset: 2, ChunkSize: 2},
	},
	)
	...
	keyPositions2 = append(keyPositions2, &KeyPosition{
		...
		key:       []byte("abc 0"),
		partition: 0,
		position:  &dwal.ChunkPosition{SegmentId: 3, BlockNumber: 3, ChunkOffset: 3, ChunkSize: 3},
	}, &KeyPosition{
		key:       []byte("key abc"),
		partition: 0,
		position:  &dwal.ChunkPosition{SegmentId: 4, BlockNumber: 4, ChunkOffset: 4, ChunkSize: 4},
	}, &KeyPosition{
		key:       []byte("abc 1"),
		partition: 0,
		position:  &dwal.ChunkPosition{SegmentId: 4, BlockNumber: 4, ChunkOffset: 4, ChunkSize: 4},
