...
func Open(options Options) (*DB, error) {
	...
	if _, err := os.Stat(options.NonceDirPath); err != nil {
	...
	index, err := openIndex(indexOptions{
		...
		dirPath:         options.NonceDirPath, //B+tree/Hash stored in trusted SSD
...
func validateOptions(options *Options) error {
	if options.DirPath == "" || options.NonceDirPath == "" {
...
func (db *DB) Compact() error {
	...
	openVlogFile := func(part int, ext string) *dwal.WAL {
		walFile, err := dwal.Open(dwal.Options{
			DirPath:        db.vlog.options.dirPath,
			NonceDirPath:   db.vlog.options.nonceDirPath,
	...
	for i := 0; i < int(db.vlog.options.partitionNum); i++ {
		...
		g.Go(func() error {
			...
			for {
				count++
				chunk, nonce, pos, err := reader.Next()
				if err != nil {
				...
				record := decodeValueLogRecord(chunk, nonce)
...
func (db *DB) rewriteValidRecords(walFile *dwal.WAL, validRecords []*ValueLogRecord, part int) error {
